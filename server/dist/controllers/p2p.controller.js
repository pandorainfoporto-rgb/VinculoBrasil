...
export const getListings = async (req, res) => {
    try {
        const { minDiscount, maxDiscount, minTenantScore, city, page = '1', limit = '20', } = req.query;
...
export const getListingById = async (req, res) => {
    try {
        ...
        const paidOnTime = listing.contract.payments.filter((p) => p.status === 'PAID' && p.paidAt && p.paidAt <= p.dueDate).length;
...
export const createListing = async (req, res) => {
    try {
        const { contractId, askingPrice, priceInMatic, priceInStable, } = req.body;
        ...
        res.status(201).json({
            ...
            data: {
                ...
                message: 'Oferta criada com sucesso! Seus recebíveis foram tokenizados e estão disponíveis para compra.',
            },
        });
    }
    catch (error) {
...
export const getMyPurchases = async (req, res) => {
    try {
        ...
        const purchasesWithProjection = purchases.map((purchase) => {
            ...
            const expectedReceipts = futurePayments.reduce((sum, p) => sum + Number(p.amount) * 0.85, // 85% vai pro investidor
            0);
            return {
                id: purchase.id,
 ...
 */
export const simulateListing = async (req, res) => {
    try {
        const { monthlyRent, monthsToSell, discountPercent, } = req.body;
 ...
 */
export const handleSaleWebhook = async (req, res) => {
    try {
 ...
 */
export const createBuyIntent = async (req, res) => {
    try {
 ...
 */
export const settlePurchase = async (req, res) => {
    try {
...
//# sourceMappingURL=p2p.controller.js.map