...
export class PinataService {
    ...
    async uploadFile(filePath, metadata) {
        try {
            ...
            const response = await client.post('/pinning/pinFileToIPFS', formData, {
    ...
    async uploadJSON(data, name) {
        try {
            const client = await this.createClient();
            const response = await client.post('/pinning/pinJSONToIPFS', {
                pinataContent: data,
                pinataMetadata: {
    ...
    async uploadInspection(params) {
        try {
            ...
            // 3. Upload do metadata JSON
            const metadataHash = await this.uploadJSON(metadata, `${params.propertyAddress}-${params.inspectionType}-metadata.json`);
