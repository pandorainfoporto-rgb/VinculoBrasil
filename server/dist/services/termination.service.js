 ...
 */
const createTerminationFineCharge = async (contract, fineAmount, investorPixKey) => {
    try {
        const api = await createAsaasClient();
        // Buscar ou criar customer no Asaas
        let asaasCustomerId = contract.financialSnapshot?.asaasCustomerId;
        if (!asaasCustomerId) {
 ...
 */
const createOwnerDebtCharge = async (contractId, owner, debtAmount, investorPixKey) => {
    try {
 ...
 */
export const calculateTermination = async (contractId, exitDate, baseFineMonths = 3) => {
 ...
 */
export const simulateTermination = async (contractId, exitDate, baseFineMonths = 3) => {
 ...
 */
export const executeTermination = async (contractId, exitDate, baseFineMonths = 3) => {
    ...
    try {
        ...
        if (calculation.proportionalFine > 0) {
            const fineCharge = await createTerminationFineCharge(contract, calculation.proportionalFine, investorPixKey);
        ...
        if (calculation.hasShortfall && calculation.ownerDebt > 0 && calculation.ownerId) {
            ...
            if (owner) {
                const debtCharge = await createOwnerDebtCharge(contractId, owner, calculation.ownerDebt, investorPixKey);
