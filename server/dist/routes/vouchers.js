    ...
    try {
        ...
        const campaigns = await prisma.voucherCampaign.findMany({
            where: {
                ...
                ...(settlementType && { settlementType: String(settlementType) }),
            },
            include: {
                partner: {
                    select: {
...
router.get('/stats', requirePermission('vouchers:read'), async (_req, res, next) => {
    try {
        const [totalCampaigns, activeCampaigns, totalRedemptions, confirmedRedemptions, burnRedemptions, transferRedemptions,] = await Promise.all([
