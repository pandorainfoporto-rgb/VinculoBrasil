...
router.post('/login', async (req, res, next) => {
    try {
        ...
        const refreshToken = jwt.sign({ userId: user.id, type: 'refresh' }, config.jwtSecret, { expiresIn: config.jwtRefreshExpiresIn });
        // Create session
...
router.post('/refresh', async (req, res, next) => {
    try {
        ...
        // Generate new token
        const newToken = jwt.sign({ userId: session.userId, email: session.user.email }, config.jwtSecret, { expiresIn: config.jwtExpiresIn });
        // Update session
