...
const decrypt = (encryptedText: string): string => {
  try {
    ...
    if (!ivHex || !encrypted) return encryptedText; // Não criptografado
...
function getCategoryForKey(key: string): string {
  if (key.includes('PINATA')) return 'STORAGE';
  if (key.includes('ASAAS')) return 'PAYMENT';
  if (key.includes('OPENAI')) return 'AI';
  if (key.includes('SMART_CONTRACT')) return 'BLOCKCHAIN';
  return 'OTHER';
}
