...
router.get('/public', async (req, res, next) => {
  try {
    ...
    if (minPrice || maxPrice) {
      where.rentValue = {};
      if (minPrice) (where.rentValue as Record<string, number>).gte = parseFloat(minPrice as string);
      if (maxPrice) (where.rentValue as Record<string, number>).lte = parseFloat(maxPrice as string);
    }
