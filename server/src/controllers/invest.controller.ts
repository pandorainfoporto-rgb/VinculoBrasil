 ...
 */
export const createInvestmentOrder = async (req: Request, res: Response) => {
  try {
    ...
    let pixData: { qrCode?: string; qrCodeImage?: string; paymentId?: string; expiresAt?: Date } = {};
...
export const processPaymentConfirmed = async (
  ...
  try {
    ...
    const settlementResult = await settleInvestment(
      order.listingId,
      order.userId,
      asaasPaymentId
    );
...
async function generateAsaasPixCharge(params: AsaasPixParams): Promise<{
  paymentId: string;
  qrCode: string;
  qrCodeImage: string;
  expiresAt: Date;
  ...
  const searchResponse = await fetch(
    `${baseUrl}/customers?email=${encodeURIComponent(params.buyer.email)}`,
    {
      headers: {
 ...
 * Endpoint leve para polling - apenas retorna o status
 */
export const getOrderStatus = async (req: Request, res: Response) => {
  try {
    ...
    logger.error(`[Invest Controller] getOrderStatus error: ${error instanceof Error ? error.message : String(error)}`);
    res.status(500).json({
      success: false,
