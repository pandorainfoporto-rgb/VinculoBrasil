...
interface CreateCategoryBody {
  name: string;
  type: 'EXPENSE' | 'INCOME';
  categoryType?: 'SYNTHETIC' | 'ANALYTICAL';
  nature?: 'DEBIT' | 'CREDIT';
  code?: string;
  description?: string;
  parentId?: string;
...
async function calculateLevelAndPath(parentId: string | null | undefined): Promise<{ level: number; path: string }> {
  if (!parentId) {
    return { level: 1, path: '' };
  }
...
async function generateAutoCode(parentId: string | null | undefined, type: 'EXPENSE' | 'INCOME'): Promise<string> {
...
function buildTree(categories: any[], parentId: string | null = null): CategoryNode[] {
  return categories
...
export async function updateCategory(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    const {
      ...
      parentId,
      isActive,
    }: Partial<CreateCategoryBody> = req.body;
 ...
 */
export async function getAnalyticalCategories(req: Request, res: Response, next: NextFunction) {
  try {
