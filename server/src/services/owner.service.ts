...
interface CreateOwnerData {
    ...
    email: string;
    phone?: string;
    cpf: string; // Used for User and Owner profile
    password?: string; // Optional, can be auto-generated

    // Owner Data
    rgIe?: string;
    birthDate?: string | Date; // Date string or object
    profession?: string;
    ...
    // Bank Data (Optional initially, but good to have)
    bankAccount?: {
        bankCode: string;
        bankName?: string;
        agencyNumber: string;
    ...
    agencyId?: string; // Optional linkage if creating via Agency context
}
...
export class OwnerService {
     ...
     */
    static async createOwner(data: CreateOwnerData) {
            ...
            if (data.bankAccount) {
                await tx.bankAccount.create({
                    data: {
                        ...
                        pixKeyType: data.bankAccount.pixKeyType as any, // Cast if enum mismatch slightly
                        active: true
                    }
                });
            }
     ...
     * Update Owner
     */
    static async updateOwner(id: string, data: Partial<CreateOwnerData>) {
