 ...
 */
export const getConfig = async (key: string, envFallback?: string): Promise<string> => {
  // 1. Verificar cache
  ...
  if (envFallback && process.env[envFallback]) {
    const envValue = process.env[envFallback] as string;
 ...
 */
export const getConfigOrDefault = async (key: string, defaultValue: string): Promise<string> => {
  try {
 ...
 */
export const getConfigs = async (keys: string[]): Promise<Record<string, string>> => {
 ...
 */
export const getConfigsByGroup = async (group: string): Promise<Array<{
  key: string;
  value: string;
  label: string | null;
 ...
 */
export const getConfigGroups = async (): Promise<string[]> => {
  try {
 ...
 */
export const deleteConfig = async (key: string): Promise<{ success: boolean; error?: string }> => {
  try {
 ...
 */
export const getBlockchainConfig = async (): Promise<{
