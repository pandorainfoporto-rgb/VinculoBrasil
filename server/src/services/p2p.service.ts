...
// ============================================
import { getConfig, getBlockchainConfig } from './config.service.js';
 ...
 */
const loadBlockchainConfig = async (): Promise<typeof cachedConfig> => {
 ...
 */
export const clearP2PConfigCache = (): void => {
  cachedConfig = null;
 ...
 */
const getP2PContract = async (signer?: ethers.Signer): Promise<ethers.Contract> => {
  const cfg = await loadBlockchainConfig();
  if (!cfg!.p2pContract) {
 ...
 */
export const tokenizeReceivables = async (input: TokenizeReceivablesInput): Promise<{
  success: boolean;
  tokenId?: string;
  txHash?: string;
  error?: string;
  ...
  try {
    ...
    const tx = await receivablesContract.mintReceivable(
      input.sellerWallet,
      input.contractId,
      input.totalMonths,
      monthlyValueWei,
 ...
 */
export const createP2PListing = async (input: CreateListingInput): Promise<{
  success: boolean;
  listingId?: string;
  dbListingId?: string;
  txHash?: string;
  ...
  try {
    ...
    const cfg = await loadBlockchainConfig();
    const receivablesContract = await getReceivablesContract(wallet);
    const isApproved = await receivablesContract.isApprovedForAll(
      input.sellerWallet,
      cfg!.p2pContract
    );
    ...
    const tx = await p2pContract.createListing(
      cfg!.receivablesContract,
      tokenResult.tokenId,
    ...
    const listingEvent = receipt.logs.find((log: ethers.Log) => {
      try {
    ...
    if (listingEvent) {
      const parsed = p2pContract.interface.parseLog({
        topics: listingEvent.topics as string[],
 ...
 */
export const handleP2PSale = async (
  listingId: string,
  buyerWallet: string,
  txHash: string
): Promise<void> => {
 ...
 */
export const getMarketplaceStats = async (): Promise<P2PMarketplaceStats> => {
  try {
    ...
    const totalVolume = soldListings.reduce(
      (sum, l) => sum + Number(l.soldPrice || 0),
      0
    );
 ...
 */
export const startP2PEventListener = async (): Promise<void> => {
  try {
    const cfg = await loadBlockchainConfig();
    if (!cfg!.p2pContract) {
