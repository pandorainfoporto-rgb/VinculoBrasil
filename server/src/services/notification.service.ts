...
// Integração com Twilio ou Z-API
// ============================================
 ...
 */
export const sendWhatsAppNotification = async (data: WhatsAppNotificationData): Promise<{
  success: boolean;
  messageId?: string;
  error?: string;
}> => {
 ...
 */
export const sendEmailNotification = async (data: EmailNotificationData): Promise<{
  success: boolean;
  messageId?: string;
  error?: string;
}> => {
 ...
 */
export const notifyInvestorsNewListing = async (listing: P2PListingData): Promise<{
  success: boolean;
  whatsappSent: number;
  emailsSent: number;
}> => {
...
<body>
  <div class="container">
    ...
    const notifications = investors.map(async (investor) => {
      ...
      if (!user) return { whatsapp: false, email: false };
...
// ============================================
// COMPROVANTE DE INVESTIMENTO (Email)
// ============================================
 ...
 */
export const sendInvestmentReceipt = async (data: InvestmentReceiptData): Promise<{
  success: boolean;
  messageId?: string;
  error?: string;
}> => {
