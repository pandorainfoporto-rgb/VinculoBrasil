...
const createAsaasClient = async (): Promise<AxiosInstance> => {
...
interface WaterfallCalculation {
  // Valores Base
  baseValue: number;      // V - O que o proprietário pediu (85%)
  totalValue: number;     // VT - O que o inquilino paga (100%)
  ecosystemPot: number;   // E - Delta que financia o sistema (15%)

  // Split do Ecossistema
  guarantorShare: number;
  suretyShare: number;
  vinculoShare: number;

  // Split Imobiliário (dentro dos 85%)
  agencyShare: number;
  ownerShare: number;
...
export const calculateWaterfall = (
  baseValue: number,
  kycScore: number = 0,
  suretyCost: number = DEFAULT_SURETY_COST,
  agencyRate: number = AGENCY_COMMISSION_RATE
 ...
 */
export const createWaterfallCharge = async (contractId: string): Promise<AsaasChargeResult> => {
  ...
  let asaasCustomerId = (contract as any).asaasCustomerId;
  ...
  const kycScore = (contract.financialSnapshot as any)?.kycScore || 0;
  ...
  const suretyCost = insuranceCompany?.fixedFee
    ...
    : (contract.financialSnapshot as any)?.suretyBondValue || DEFAULT_SURETY_COST;
  ...
  let beneficiaryWalletId: string | null | undefined = ownerPixKey;
...
export const simulateWaterfall = (
  ...
  suretyCost: number = DEFAULT_SURETY_COST,
  agencyRate: number = AGENCY_COMMISSION_RATE
): WaterfallCalculation & { breakdown: string[] } => {
 ...
 */
export const generateMonthlyCharges = async (month: number, year: number): Promise<{
  created: number;
  skipped: number;
  errors: string[];
}> => {
