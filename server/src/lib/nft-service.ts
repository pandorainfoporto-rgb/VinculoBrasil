...
interface NFTMintParams {
  ownerAddress: string; // Endereço da carteira que receberá o NFT
...
export class NFTService {
   ...
   */
  async mintInspectionNFT(params: NFTMintParams): Promise<{
    tokenId: number;
    ...
    try {
      ...
      const mintEvent = receipt.logs
        ...
        .find((event: any) => event && event.name === 'PropertyNFTMinted');
   ...
   */
  async estimateMintGas(params: NFTMintParams): Promise<{
    gasLimit: string;
    ...
    try {
      ...
      const gasEstimate = await this.contract.safeMint.estimateGas(
        params.ownerAddress,
        params.propertyAddress,
        params.inspectionType,
        'ipfs://dummy' // URI dummy para estimativa
      );
